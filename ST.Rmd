...
![Exemplo de *overfitting*](img/overfiting.PNG)
...
![Uma representação do equilíbrio entre flexibilidade e interpretabilidade, usando diferentes métodos de aprendizagem estatística. Em geral, à medida que a flexibilidade de um método aumenta, sua interpretabilidade diminui.](img/2-7.png)
...
![*Mean Squared Error* entre as variáveis *Sales* e *TV*, do dataset *Advertising*. Cada segmento de linha cinza representa um erro, e o ajuste faz um compromisso calculando a média de seus quadrados. Neste caso, um ajuste linear captura a essência de a relação, embora seja um pouco deficiente.](img/mse.PNG)
...
Se realmente não houver relação entre $X$ e $Y$, então esperamos que a equação tenha uma distribuição $t$ com $n - 2$ graus de liberdade. A distribuição $t$ tem uma forma de sino e para valores de $n$ maiores que aproximadamente $30$ é bastante semelhante à distribuição normal.
...
![*Mean Squared Error* entre as variáveis *Sales* e *TV*, do dataset *Advertising*. Cada segmento de linha cinza representa um erro.](img/mse.PNG)
...
![Gráficos de resíduos versus valores preditos (ou ajustados) para o conjunto de dados Auto. Em cada gráfico, a linha vermelha é um ajuste suave para os resíduos, com o objetivo de facilitar a identificação de uma tendência. Esquerda: Uma regressão linear de *mpg* em *horsepower*. Uma forte padrão nos resíduos indica não linearidade nos dados. À direita: uma regressão linear de *mpg* em *horsepower* e $horsepower^2$. Existe um pequeno padrão no residuais.](img/3-9.png)

2. **Correlação de termos de erro** (contexto de séries temporais): resíduos não podem apresentar padrões

![Gráficos de resíduos de conjuntos de dados de série temporal simulados gerados com diferentes níveis de correlação entre termos de erro para pontos de tempo adjacentes.](img/3-10.png)
...
![A linha de regressão de mínimos quadrados é mostrada em vermelho, e o a linha de regressão após a remoção do outlier é mostrada em azul.](img/3-3.PNG)
...
# A biblioteca MASS contém o conjunto de dados de Boston, que registra medv (valor médio da casa) para 506 bairros ao redor de Boston. Buscaremos prever medv utilizando 13 preditores, como rm (número médio de quartos por casa), idade (idade média das casas) e lstat (percentual de domicílios com baixo nível socioeconômico).
...
# O valor de p próximo de zero associado ao termo quadrático sugere que ele leva a um modelo aprimorado. Usamos a função anova() para quantificar ainda mais a extensão em que o ajuste quadrático é superior ao ajuste linear.
...
# Vemos que usando a seleção progressiva, o melhor modelo de uma variável contém apenas CRBI, e o melhor modelo de duas variáveis inclui adicionalmente Hits
...
![Linhas de contorno das funções de erro e restrição para o LASSO (esquerda) e Ridge Regression (direita). As áreas em azul sólido são as regiões de restrição, $|\beta_1| + |\beta_2| \leq s$ e $\beta_1^2 + \beta_2^2 \leq s$, enquanto as elipses vermelhas são os contornos de RSS.](img/6-7.png)

![Ridge Regression e as estimativas de LASSO para uma configuração simples com $n = p$ e $X$ uma matriz diagonal unitária. Esquerda: As estimativas de coeficiente de Ridge Regression são reduzidas proporcionalmente para zero, em relação às estimativas de mínimos quadrados. Direita: As estimativas de LASSO são suavizadas para zero](img/6-10.PNG)
...
# Podemos ver no gráfico de coeficiente que dependendo da escolha do parâmetro de ajuste, alguns dos coeficientes serão exatamente iguais a zero.

# Cada curva corresponde a uma variável. Ele mostra o caminho de seu coeficiente em relação à norma \ell_1 de todo o vetor de coeficientes à medida que \lambda varia. O eixo acima indica o número de coeficientes diferentes de zero no \lambda atual, que são os graus de liberdade efetivos X para o LASSo
lasso.mod = glmnet(x, y, alpha=1, lambda=grid)
suppressWarnings(plot(lasso.mod))

# cv.glmnet() executa validação cruzada dez vezes
cv.out.lasso = cv.glmnet(x[train,], y[train], alpha=1)

# Inclui a curva de validação cruzada (linha pontilhada vermelha), e curvas de desvio padrão superior e inferior ao longo da sequência de \lambda. Dois \lambda são selecionados e indicados pelas linhas pontilhadas verticais.
plot(cv.out.lasso)
...
suppressWarnings(plot(ridge.mod))
...
![A linha sólida verde indica o primeiro componente principal, e a linha tracejada azul indica o segundo componente principal.](img/6-14.png)
...
![Esquerda: um screeplot representando a proporção da variância explicada por cada um dos quatro componentes principais nos dados USArests. À direita: a proporção cumulativa da variância explicada pelos quatro componentes principais no USArests.](img/10-4.png)
...
![Esquerda: A curva sólida exibe o valor ajustado de uma regressão de mínimos quadrados usando funções de passo. As curvas pontilhadas indicam um intervalo de confiança estimado de 95%. À direita: regressão logística, novamente usando funções de passo.](img/7-2.png)

## Polinômios por partes

A cada região uma interpolação polinomial.

![Superior esquerdo: Polinômios cúbicos irrestritos. Superior direito: Polinômios cúbicos restritos a serem contínuos em Age = 50. Inferior esquerdo: Polinômios cúbicos restritos a serem contínuos e ter primeira e segunda derivadas contínuas. Inferior direito: Spline linear a qual é restrita a ser contínua.](img/7-3.png)
...
![Spline cúbica e Spline cúbica natural, com três nós.](img/7-4.png)

Obs: Splines naturais são utilizados para minimizar a variância.

![Spline versus polinômio, ambos com 15 graus de liberdade.](img/7-7.png)
